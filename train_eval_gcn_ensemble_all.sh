...
run_script() {
    if [ "$4" == "random" ]; then
        python3 "train_and_eval.py" with "configs/reference/${1}_gcn_ensemble.yaml" \
        "run.num_splits=10" "run.gpu=$2" "data.dataset=$3" "data.split=$4" "data.ood_left_out_classes=$5" &
    elif [ "$4" == "public" ]; then
        python3 "train_and_eval.py" with "configs/reference/${1}_gcn_ensemble.yaml" \
